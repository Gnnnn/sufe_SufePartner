...
if($request=="view"){
  ...
  if(!$username){
    ...
    echo json_encode(array("isLogin"=>0));  
      ...
      $result2 = mysqli_query($con,"SELECT `helper` from qusid_helper where qus2id = {$value};");
      if(!result2){
        $helpernum = 0;
      }
      else{
...
if($request=="besthelper"){
  ...
  if(!$username){
    echo json_encode(array("isLogin"=>0));  
      ...
      $con = new mysqli($dbip,$usn,$psw,$dbname);
      mysqli_query($con,"SET NAMES 'UTF8'");
      $result=mysqli_query($con,"SELECT helper,count(qus2id) as count from qusid_helper group by helper order by count(qus2id) desc;");
...
if($request=="qusview"){
  ...
  if(!$username){
    echo json_encode(array("isLogin"=>0));  
      ...
      $row = mysqli_fetch_array($result);
      $result2 = mysqli_query($con,"SELECT `helper` from qusid_helper where qus2id = {$_SESSION["thisqus2id"]};");
...
if($request=="myans"){
  ...
  if(!$username){
    echo "<script>self.location='../index.html'</script>";  
      ...
      $result = mysqli_query($con,"INSERT into qusid_helper set qus2id = '{$_SESSION["thisqus2id"]}',helper='{$_SESSION[username]}',ans='{$myans}',`time`='{$timenow}';");
      $result = mysqli_query($con,"SELECT ans from qusid_helper where helper = '{$_SESSION[username]}';");
      $row = mysqli_fetch_array($result);
      // echo $row[ans];
      if($row[ans]!=$myans){
      echo json_encode(array("isUpdate"=>0));
...
if($request=="ansview"){
  ...
  if(!$username){
    echo json_encode(array("isLogin"=>0));  
      ...
      $result = mysqli_query($con,"SELECT helper from qusid_helper where qus2id={$_SESSION["thisqus2id"]};");
      ...
      foreach ($temp as $value) {
      $result = mysqli_query($con,"SELECT `ans`from qusid_helper where helper = '{$value}' and qus2id={$_SESSION["thisqus2id"]};");
      $row = mysqli_fetch_array($result);
